...
TYPE(Tnsf_variable),DIMENSION(3,26)     :: SELFE_out_vars;
INTEGER(intKind)   ,DIMENSION(26)       :: SELFE_out_vars_ndim;
!--------------------------------------------------------------
...
INTERFACE nfs_self_write_var2file
 MODULE PROCEDURE  nfs_self_write_var2file_2d_scalar;
 MODULE PROCEDURE  nfs_self_write_var2file_3d_scalar;
END INTERFACE   
...
INTEGER(intKind)               ,INTENT(IN  )::iterm;

INTEGER(intKind)                   ::var_id,ierr,status;
REAL(kind=rkind),DIMENSION(:),ALLOCATABLE :: data2d;
...
REAL(kind=rkind),DIMENSION(:,:),ALLOCATABLE ::data3d;
...
!--------------get the var_id; 
    ...
    IF(nsf_float == NF90_FLOAT) THEN     
      status = nf90_put_var(ncid,var_id,REAL(data3d,4), &
                       start=(/1,1,iterm/),count=(/nvrt,np_global,1/));
    ELSEIF(nsf_float == NF90_DOUBLE) THEN 
...
!------------------------------------------------------------------------------------------------------------------
!
!  Purpose      : create the output netcdf file for SELFE. Note this program is only executed on Rank 0.
!                  
!                 
!                 
!  Input        : 
...
!  close the file when it created.
    CALL nfs_close_file(nsf_self);
ENDIF

!------------------------------------------------------------------
!Before create the file, compute the local to global node mapping.
! which is used for data gathering:nsf_np,nsf_np_index and nsf_ipgl;
!The following array can be allocated only on rank 0 , in current version
!  we allocate on all rank.
!------------------------------------------------------------------
...
!set the output time step for current netcdf file as 0;                 
nsf_out_ntime = 0;
...
!--------------SET the global attribute of the SELFE---------------------------------------
...
!-----------------set the array dimension in the netcdf file----------------------
...
!    out put the x 
      ...
      DO i = 1,np
          global_index =iplg(i);
          istat = nf90_put_var(nsf_self_ncid,temp_var_id,REAL(xnd(i:i),4), &
                            (/global_index/),(/1/));                    
...
!    out put the y 
      ...
      DO i = 1,np
          global_index =iplg(i);
          istat = nf90_put_var(nsf_self_ncid,temp_var_id,REAL(ynd(i:i),4), &
                           (/global_index/),(/1/));                    
          CALL  check_error(istat)      
      ENDDO
...
!    out put the node xlon 
      ...
      DO i = 1,np
          global_index =iplg(i);
          istat = nf90_put_var(nsf_self_ncid,temp_var_id,REAL(xlon(i:i),4), &
                           (/global_index/),(/1/));                    
          CALL  check_error(istat) 
      ENDDO
...
!    out put the node ylat 
      ...
      DO i = 1,np
          global_index =iplg(i);
          istat = nf90_put_var(nsf_self_ncid,temp_var_id,REAL(ylat(i:i),4), &
                           (/global_index/),(/1/));                    
          CALL  check_error(istat) 
      ENDDO
