...
! Check bound
    ...
    enddo
    if(ie<=ne) then
      write(10,'(a,2i8,3e14.6)') 'Element ',ie,ielg(ie),xctr(ie),yctr(ie),dpe(ie)
    else
      write(10,'(a,2i8,3e14.6)') '# Element ',ie,ielg(ie),xctr(ie),yctr(ie),dpe(ie)
    endif
...
!        ibuf3(k)=0
    ...
    enddo
    if(ip<=np) then
      write(10,'(a,2i8,3e14.6,2i4,50(i8,i4))') 'Node ',ip,iplg(ip),x(ip),y(ip),dp(ip), &
  ...
  do isd=1,nsa
    ...
    enddo
    if(isd<=ns) then
      write(10,'(a,6i8,6e14.6,i4)') 'Side ',isd,isdgb,ngb1,ngb2,iegb1,iegb2, &
      xcj(isd),ycj(isd),dps(isd),distj(isd),snx(isd),sny(isd),isbs(isd)
    else
      write(10,'(a,6i8,6e14.6,i4)') '# Side', isd,isdgb,ngb1,ngb2,iegb1,iegb2, &
      xcj(isd),ycj(isd),dps(isd),distj(isd),snx(isd),sny(isd),isbs(isd)
    endif
  ...
  ! All ranks write local to global element and node info
...
!-------------------------------------------------------------------------------
  ...
  !  ielg; nm; ic3; nne; ine; ne; nea; x; y; dp
  ...
  do ie=1,ne
    ...
    do j=1,3
      ip=nm(ie,j)
      do k=1,nne(ip)
        jegb=ielg(ine(ip,k)) !ine(ip,k) inside aug. domain
        if(jegb/=iegb) then
          found=.false.
          do l=1,nedge
  ...
  ! Number of weights associated with each vertex
  ncon = 4
...
!  int(out) part(): array size = # of local vertices. It stores indices of local vertices.
  ...
  deallocate(xyz,nlev,tpwgts,ubvec)
...
!  real(rkind),parameter :: pi=3.141592653589793d0
  ...
  integer :: ics,n1,n2,n3,n4,jsj,nt,nn,nscnt,nsgcnt
  real(rkind) :: slam0,sfea0,ar1,ar2,ar3,ar4,signa
  real(rkind) :: xtmp,ytmp,dptmp,thetan,egb1,egb2,egb
  integer :: ipre !indvel is in module elfe_glbl
...
!-------------------------------------------------------------------------------
  ...
  ! Get required information from parameter file
!  open(15,file='param.in',status='old',iostat=stat)
!  if(stat/=0) call parallel_abort('AQUIRE_HGRID: open(15) failure')
!  read(15,'(a48)') !version
!  read(15,'(a48)') !start_time
!  read(15,*) ipre !pre-processing flag -- single processor only
  ...
  call get_param('cpp_lon',2,intvalue,slam0,stringvalue)
  call get_param('cpp_lat',2,intvalue,sfea0,stringvalue)
  slam0=slam0*deg2rad
  sfea0=sfea0*deg2rad
  !End reading param.in for now
...
!  if(stat/=0) call parallel_abort('AQUIRE_HGRID: i34gb allocation failure')
  ...
  do iegb=1,ne_global
    do k=1,3
      ic3gb(iegb,k)=0 !index for boundary sides
...
! inegb to be re-arrange in counter-clockwise fashion after boundary info is read in
  ...
  do iegb=1,ne_global
    do j=1,3 !visit each side associated with element iegb
      if(ic3gb(iegb,j)==0.or.iegb<ic3gb(iegb,j)) then !new global side
        ns_global=ns_global+1
        jsgb(iegb,j)=ns_global
        if(ic3gb(iegb,j)/=0) then !old internal side
          jegb=ic3gb(iegb,j)
          l=0
          do k=1,3
        ...
        endif !ic3gb(iegb,j)/=0
      endif !ic3gb(iegb,j)==0.or.iegb<ic3gb(iegb,j)
    enddo !j
  enddo !iegb
  ...
  if(full_aquire) then
    if(myrank==0) then
      write(16,'(/a,4i10)') &
      'Global Grid Size (ne,np,ns,nvrt): ',ne_global,np_global,ns_global,nvrt
    endif
  ...
  ! Compute total lengths of each _global_ open bnd segments (for imposing discharge)
  if(full_aquire) then
    if(allocated(cwidth)) deallocate(cwidth);
    allocate(cwidth(nope_global),stat=stat);
    if(stat/=0) call parallel_abort('AQUIRE_HGRID: cwidth allocation failure')
    do i=1,nope_global
      cwidth(i)=0
      do j=1,nond_global(i)-1
        ii=iond_global(i,j)
        jj=iond_global(i,j+1)
        cwidth(i)=cwidth(i)+sqrt((x_global(jj)-x_global(ii))**2+(y_global(jj)-y_global(ii))**2)
      enddo !j
    enddo !i
  endif
  ...
  ! Allocate and classify global boundary sides
  ...
  if(nproc>1) then
    do iegb=1,ne_global
      ...
      do k=1,3 !visit each node/side attached to element iegb
        !Node table
        ipgb=nmgb(iegb,k) !ipgb is global node index
...
! nodep%next=>null()
        ...
        !Side table
        isgb=jsgb(iegb,k) !isgb is global side index
...
!    i34(ie)=3 !i34gb(iegb)
    do j=1,3
      ...
      jegb=ic3gb(iegb,j)
      if(jegb/=0.and.iegl(jegb)%rank==myrank) then
...
!  if(allocated(inegb)) deallocate(inegb)
  ...
  if(allocated(x)) deallocate(x); allocate(x(npa),stat=stat);
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: x allocation failure')
  if(allocated(y)) deallocate(y); allocate(y(npa),stat=stat);
  ...
  do i=1,np_global
    ...
    if(node%rank==myrank) then
      if(ics==1) then
        ...
        xlon(node%id)=xtmp*deg2rad
        ylat(node%id)=ytmp*deg2rad
        dp(node%id)=dptmp
        call cpp(x(node%id),y(node%id),xlon(node%id),ylat(node%id),slam0,sfea0)
      endif !ics
!      if(dp(node%id).le.0) idrynode(node%id)=1
  ...
  if(full_aquire) then
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
  ! Compute element areas, equivalent radii 
  ...
  do ie=1,nea
    ...
    n2=nm(ie,2)
    n3=nm(ie,3)
    area(ie)=signa(x(n1),x(n2),x(n3),y(n1),y(n2),y(n3))
    if(area(ie)<=0.d0) then
      write(errmsg,'(a,2i8)') 'AQUIRE_HGRID: ERROR: negative area at',ie,ielg(ie)
      call parallel_abort(errmsg)
    endif 
  ...
  do ie=1,nea
    ...
    do j=1,3
      ...
      n2=nm(ie,nx(j,2))
      if( ic3(ie,j)==0 &
      .or.(ic3(ie,j)>0.and.iegb<ielg(ic3(ie,j))) &
      .or.(ic3(ie,j)<0.and.iegb< abs(ic3(ie,j)))) then !new local side
        is(jsj,1)=ie
        is(jsj,2)=ic3(ie,j)
        isidenode(jsj,1)=n1
        isidenode(jsj,2)=n2
      ...
      xcj(jsj)=(x(n1)+x(n2))/2d0
      ycj(jsj)=(y(n1)+y(n2))/2d0
      dps(jsj)=(dp(n1)+dp(n2))/2d0
      distj(jsj)=sqrt((x(n2)-x(n1))**2+(y(n2)-y(n1))**2)
      if(distj(jsj)==0) then
  ...
  ! Allocate and compute sign (orientation of normal velocity) associated
  ...
  ! Allocate and compute element centers and depths
...
!  if(allocated(xlon_e)) deallocate(xlon_e); allocate(xlon_e(nea),stat=stat);
!  if(stat/=0) call parallel_abort('AQUIRE_HGRID: xlon_e allocation failure')
!  if(allocated(ylat_e)) deallocate(ylat_e); allocate(ylat_e(nea),stat=stat);
!  if(stat/=0) call parallel_abort('AQUIRE_HGRID: ylat_e allocation failure')
  do ie=1,nea
    xctr(ie)=0d0
    yctr(ie)=0d0
    dpe(ie)=1.d10
    do j=1,3
      xctr(ie)=xctr(ie)+x(nm(ie,j))/3.d0
      yctr(ie)=yctr(ie)+y(nm(ie,j))/3.d0
      if(dp(nm(ie,j))<dpe(ie)) dpe(ie)=dp(nm(ie,j))
    enddo !j
  enddo !ie

  ! Allocate and compute angles (orientation of local x) 
  if(allocated(snx)) deallocate(snx); allocate(snx(nsa),stat=stat);
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: snx allocation failure')
  if(allocated(sny)) deallocate(sny); allocate(sny(nsa),stat=stat);
  if(stat/=0) call parallel_abort('AQUIRE_HGRID: sny allocation failure')
  do j=1,nsa
    n1=isidenode(j,1)
    n2=isidenode(j,2)
