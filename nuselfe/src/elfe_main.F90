...
!     Non-hydrostatic arrays
      allocatable :: qnon(:,:),qhat(:,:),dqnon_dxy(:,:,:),qmatr(:,:,:,:),qir(:,:)
...
!     Jet model. a - ambient, j - jet, 0 - original
      integer :: ijet,nofe_max
      real(rkind) :: R2nej,Qj0,Dj0,Tj0,Sj0,Cj0,xj0,yj0,zj0,phij0,thetaj0
...
!==============================================================================
!     For wave models, bypass initialization if not forst call
!==============================================================================
      if(first_call) then
...
!     Allocate the remaining grid geometry arrays held in elfe_glbl
      allocate(kbe(nea),idry_e(nea),idry_e0(nea),interpol(nea),lqk(nea),ie_kr(nea),krvel(nea),ze(nvrt,nea),dl(nea,3,2), &
           dp00(npa),kfp(npa),kbp(npa),kbp00(npa),kbp_e(np),idry(npa),iback(npa),hmod(npa),z(nvrt,npa), &
...
!     Tracers
      if(ntracers==0) then !allocate some trivial arrays (for reference only)
        ...
        allocate(trel0(ntracers,nvrt,nea),trel(ntracers,nvrt,nea), &
                 tr_nd(ntracers,nvrt,np),trth(ntracers,max(1,nope_global)),stat=istat)
      endif
      if(istat/=0) call parallel_abort('MAIN: other allocation failure')
...
!     Initialize some arrays and constants
      tempmin=0; tempmax=40; saltmin=0; saltmax=62 !42
...
!          open(55,file='uv3D.th',status='old')
          ...
          do j=1,nond_global(k)
            read(31,*) uthnd(k,j,1:nvrt) !used to denote normal vel.
          enddo !j
        ...
        if(inu_tr/=0) then
          ...
          do i=1,np_global
            ...
            if(tmp1<0.or.tmp1>1.or.tmp2<0.or.tmp2>1) then
              write(errmsg,*)'Wrong nudging factor at node (1):',i,tmp1,tmp2
              call parallel_abort(errmsg)
            endif
...
!     Read Jet model parameter
      ijet = 0
      open(17,file='jet.in',IOSTAT=ierr)
      if (ierr==0) then
         read(17,*)ijet
         if (ijet>0) then
            read(17,*)dtj
            ...
            read(17,*)Sj0
            read(17,*)Cj0
            read(17,*)icsj,xj0,yj0,zj0
            if(icsj==2) then
               xj0 = xj0*pi/180
               yj0 = yj0*pi/180
               call cpp(xj0,yj0,xj0,yj0,slam0,sfea0)
            endif   
            read(17,*)phij0,thetaj0
            R2nej = dt*(2.+Qj0/(pi*Dj0*Dj0/4.))
            R2nej = R2nej*R2nej
         endif
      endif
...
!...      Reads constant atmospheric parameters (!MFR - to be used when nws=0... to clean later...)
          if(nws/=0) then
            call parallel_abort('MAIN: EcoSim uses nws=0')
          else !nws=0
            if(myrank==0) write(16,*) 'Reading atmospheric parameters from atmos.in...'
!'       
...
!     Initialize Jet model array
      if (ijet>0) then
         nofe_max = nint(pi*R2nej/minval(area(1:nea)))
         Ntj = nint(dt/dtj)
         dtj = dt/Ntj
         allocate(ofe(nofe_max),stat=istat)
         if(istat/=0) call parallel_abort('MAIN: Jet model allocation failure.')
         ! List elements within a dist Rnej to the outfall
         nofe = 0
         do i=1,nea  !local index
            n1=nm(i,1)
            n2=nm(i,2)
            n3=nm(i,3)
            area0=2.*area(i) 
            ...
            x3 = x(n3)
            y3 = y(n3)
            dist = min((xj0-x1)**2+(yj0-y1)**2, &
                       (xj0-x2)**2+(yj0-y2)**2, &
                       (xj0-x3)**2+(yj0-y3)**2)
            area1=abs(x1*y2+x2*yj0+xj0*y1-x1*yj0-xj0*y2-x2*y1)
            area2=abs(x1*yj0+xj0*y3+x3*y1-x1*y3-x3*yj0-xj0*y1)
            area3=abs(xj0*y2+x2*y3+x3*yj0-xj0*y3-x3*y2-x2*yj0)
            area3=abs(area3+area2+area1-area0)/area0
            if (dist.le.R2nej.or.area3.le.1.e-4) then
               nofe = nofe+1
               ofe(nofe) = i
            endif
         enddo
         if (nofe.gt.0) then
            allocate(ej(Ntj,2),iej(nvrt,nofe),xMT(nvrt,nofe),xMS(nvrt,nofe),xMC(nvrt,nofe), &
                     xMV(nvrt,nofe),Taj(Ntj),Saj(Ntj),Caj(Ntj),rhoaj(Ntj),Volj(Ntj),&
!                    xj(Ntj),yj(Ntj),zj(Ntj),bj(Ntj),hj(Ntj),Tj(Ntj),Sj(Ntj),Cj(Ntj), &
!                    Mj(Ntj),dMj(Ntj),Mjg(Ntj),Qjg(Ntj),Maj(Ntj),Qaj(Ntj), &
                     stat=istat)
            write(myrank+1000,'(a)')'**********Discharge parameters**********'
            write(myrank+1000,*)'dtj = ',dtj
            write(myrank+1000,*)'Ntj = ',Ntj
            write(myrank+1000,*)'Qj0 = ',Qj0
            write(myrank+1000,*)'Dj0 = ',Dj0
            write(myrank+1000,*)'Tj0 = ',Tj0
            write(myrank+1000,*)'Sj0 = ',Sj0
            write(myrank+1000,*)'Cj0 = ',Cj0
            write(myrank+1000,*)'x,y,z,phi,theta,R = '
            write(myrank+1000,*)xj0,yj0,zj0
            write(myrank+1000,*)phij0,thetaj0,sqrt(R2nej)
            write(myrank+1000,'(a)')'nofe,(ielg(ofe(i)),i=1,nofe) = '
            write(myrank+1000,*)nofe,(ielg(ofe(i)),i=1,nofe)
            write(myrank+1000,*)
            if(istat/=0) call parallel_abort('MAIN: Jet model allocation failure.')
         endif
         if(myrank==0) then
            write(16,*)
            write(16,'(a)')'**********Discharge parameters**********'
            write(16,*)'dtj = ',dtj
            write(16,*)'Ntj = ',Ntj
            write(16,*)'Qj0 = ',Qj0
            write(16,*)'Dj0 = ',Dj0
            ...
            write(16,*)'Sj0 = ',Sj0
            write(16,*)'Cj0 = ',Cj0
            write(16,*)'x,y,z,phi,theta,R = '
            write(16,*)xj0,yj0,zj0
            write(16,*)phij0,thetaj0,sqrt(R2nej)
            write(16,'(a)')'nofe,(ielg(ofe(i)),i=1,nofe) in fort.myrank+1000'
            write(16,*)
            !i=ofe(nofe)
            !write(myrank+1000,*) i,xctr(i),yctr(i)
            !write(myrank+1000,*) nm(i,1),nm(i,2),nm(i,3)
            !write(myrank+1000,*) x(nm(i,1)),y(nm(i,1))
            !write(myrank+1000,*) iplg(nm(i,1)),iplg(nm(i,2)),iplg(nm(i,3))
            !write(myrank+1000,*) x(iplg(nm(i,1))),y(iplg(nm(i,1))) !wrong
            !write(myrank+1000,*) area(i)
            !write(myrank+1000,*) ze(1,i),ze(2,i)
         endif
      endif !ijet/=0
 !
 !
...
!--------------------------------------------------------
!TODO Xu add: For Non-netcdf output
!--------------------------------------------------------
IF(iwrite == 0 .OR. iwrite ==1 ) THEN       
      do i=1,noutput
        ichan(i)=100+i !output channel #

        if (ichan(i)==116) print*,'ichan is',ichan(i)!HUY
...
!         uthnd is the normal vel.; no ramp up
          do k=1,nvrt
            ...
            uth(i,k)=(uthnd(ibnd,nwild(1),k)+uthnd(ibnd,nwild(2),k))/2*snx(i)
            vth(i,k)=(uthnd(ibnd,nwild(1),k)+uthnd(ibnd,nwild(2),k))/2*sny(i)
          enddo !k
...
!     Initialize inter-subdomain backtracking count
      ...
      do l=1,3 !nodes or sides or elements
...
!             Perturb starting pt to avoid underflow (except for wvel)
              if(l<=2) then
                xt=(1-small2)*xt+small2*xctr(nnel)
                yt=(1-small2)*yt+small2*yctr(nnel)
              endif
...
!-------------------------------------------------------------------------------
! Jet model by My Ha
! Output: Qs to add to the RHS of continuity eq
!         Qc to add to the RHS of T,S and transport eq
!-------------------------------------------------------------------------------
!-------------------------------------------------------------------------------
!write(16,*)'solving jet 2' 
!if (nofe.gt.0) call jet_model(Qj0,Dj0,Tj0,Sj0,Cj0,xj0,yj0,zj0,phij0,thetaj0,grav,myrank,it)
...
!	    I_3 
            if(isbs(isd)>0.and.ifltype(isbs(isd))/=0) then !.and.(.not.lelbc(i))) then 
...
!     Compute bottom index for sides for zeroing out fluxes
...
!       Wet
        do l=1,2 !element
          ie=is(j,l)
          if(ie/=0.and.idry_e(ie)==0.and.kbe(ie)>kbs_e(j)) kbs_e(j)=kbe(ie)
        enddo !l
        if(kbs_e(j)==0) then
...
!          enddo !l
          !if(khh2==0) then
!          if(khh2/=kbs_e(j)) then
!            write(errmsg,*)'Cannot find the higher bottom (2):',j,ielg(is(j,1:2)),is(j,1:2),khh2,kbs_e(j)
!            call parallel_abort(errmsg)
!          endif
          !if(kbs(j)>khh2) then
...
!       No exchange for qhat afterwards so consistency may not be guarenteed
        call solve_jcg_qnon(it,moitn,mxitn,rtol,nvrt,mnei,np,npa,qmatr,qhat,qir)
...
!...  For hydrostatic model, this is the only vertical vel; for non-hydrostatic
!...  model, this is only used in upwind transport
      we_fv=0 !for dry and below bottom levels
      do i=1,nea
        if(idry_e(i)==1) cycle
...
!       Error: reconcile two cases later
!       we_fv=0 unless Cd=0
        if(nonhydro==1) then
...
!   	       b.c. to be imposed at the end
             do k=kbb,nvrt
               ...
               if(k<nvrt) then 
                 ...
                 if(ihconsv/=0) then !heat flux Heat exchange
                   if(l==1) then
                 ...
                 if(isconsv/=0) then !salt flux Salt exchange
                   if(l==1) then
                     rrhs(kin,2)=rrhs(kin,2)+dt/rho0*ptbt(2,k,nd0)*(fluxevp(nd0)-fluxprc(nd0))
                   else
                     rrhs(kin,2)=rrhs(kin,2)+dt/rho0*sdbt(4,k,isd0)* &
                     & (fluxevp(node1)+fluxevp(node2)-fluxprc(node1)-fluxprc(node2))/2
                   endif  !MyHa: why snd, ssd are used here. They've been reset just before ELM starts
                 endif    !change to ptbt(2,k,nd0) for snd(k,nd0) and sdbt(4,k,isd0) for ssd(k,isd0)
               endif
...
!              7: 0.80 0.90 2.1 (Mike Z.'s choice for estuary)
               ...
               if(ihconsv/=0) then !solar
                 if(l==1) then
                   zz1=max(-5.e2_rkind,z(k,nd0)-z(nvrt,nd0))
...
!        Jet model should go here to update tnd,snd,tsd,ssd
         if (nofe.gt.0) call jet_model(Qj0,Dj0,Tj0,Sj0,Cj0,xj0,yj0,zj0,phij0,thetaj0,grav,myrank,it)
...
!++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      endif !if(iupwind_t==0.or.iupwind_s==0)
!     end of ELM option
...
!         Extend
        ...
        enddo !i=1,nea Impose b.c. & nudging

!       Jet model should go here to update tsel 
        if (nofe.gt.0) call jet_model(Qj0,Dj0,Tj0,Sj0,Cj0,xj0,yj0,zj0,phij0,thetaj0,grav,myrank,it)
...
!       Exchange
        if(iupwind_t/=0.and.iupwind_s/=0) then  !note: it is "and" not "or"
...
!'
          ...
          wtimer(9,2)=wtimer(9,2)+mpi_wtime()-cwtmp
#endif
        endif !Exchange (iupwind_t/=0.and.iupwind_s/=0)
...
IF(mod(it,nspool)==0.AND. myrank ==0) THEN
