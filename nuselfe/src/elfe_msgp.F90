!===============================================================================
!===============================================================================
! SELFE PARALLEL MESSAGE PASSING MODULE
!===============================================================================
!===============================================================================
module elfe_msgp
  ...
  use elfe_glbl, only : rkind, llist_type,nvrt, &
                          ...
                          np_global,np,npg,npa,iplg,ipgl,nne,ine,x,y,dp, &
...
!  integer,public,save :: rtype = MPI_REAL8       ! MPI Real Type -- to match rkind
  integer,public,save :: rtype = MPI_DOUBLE_PRECISION       ! MPI Real Type -- to match rkind
#endif
...
! Following are arrays (:,:,:) exchange types
  ...
  integer,save,allocatable :: p3d_tr_send_type(:)   ! 3Dxntracers node send MPI datatype for tracers
...
!  public :: parallel_init           ! Initialize parallel environment
  ...
  public :: exchange_p3d_tr         ! 3Dxntracers ghost node exchange of type (ntracers,nvrt,nm) where nm>=npa
...
!  call mpi_type_indexed(nesend(i),eblen_send(1,i),edspl_send(1,i),rtype,e2dsend_type(i),ierr)
!  arguments in order:
!  inputs:
...
!    It will send selected entries of e2d_data to rank nbrrank(i).
  ...
  do i=1,nnbr_p
    if(npsend(i)/=0) then
      !Send
      blen_send(:)=1
      dspl_send(:)=ipsend(:,i)-1
      ! Note: npsend etc. can be 0
#if MPIVERSION==1
...
if(ntracers>0) then
  ...
  do i=1,nnbr_p
    if(nssend(i)/=0) then
      ...
      dspl_send(:)=(issend(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
    ...
    if(nsrecv(i)/=0) then
      ...
      dspl_recv(:)=(isrecv(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
  ...
  if(ntracers>0) then
   ...
   do i=1,nnbr_p
      if(npsend(i)/=0) then
        !Send
        blen_send(:)=nvrt*ntracers
        dspl_send(:)=(ipsend(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
      ...
      if(nprecv(i)/=0) then
        !Recv
        blen_recv(:)=nvrt*ntracers
        dspl_recv(:)=(iprecv(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
  ...
  do i=1,nnbr
    !Send
    blen_send(:)=nvrt*mntr
    dspl_send(:)=(iesend(:,i)-1)*nvrt*mntr
#if MPIVERSION==1
    ...
    !Recv
    blen_recv(:)=nvrt*mntr
    dspl_recv(:)=(ierecv(:,i)-1)*nvrt*mntr
#if MPIVERSION==1
  ...
  do i=1,nnbr
    !Send
    blen_send(:)=nvrt*ntracers
    dspl_send(:)=(iesend(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
    ...
    !Recv
    blen_recv(:)=nvrt*ntracers
    dspl_recv(:)=(ierecv(:,i)-1)*nvrt*ntracers
#if MPIVERSION==1
